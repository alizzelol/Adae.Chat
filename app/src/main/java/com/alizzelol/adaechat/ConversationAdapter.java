    ...
    public void onBindViewHolder(@NonNull ConversationViewHolder holder, int position) {
        ...
        if (conversation.getLastTimestamp() != null) {
            String formattedDate = new SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.getDefault())
                    .format(conversation.getLastTimestamp()); // Ya es Date, no necesita .toDate()
            holder.tvLastTimestamp.setText(formattedDate);
        } else {
            holder.tvLastTimestamp.setText(""); // Si es null, muestra vacío
        }


        // Establecer el tag del botón de eliminación
